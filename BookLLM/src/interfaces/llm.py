...
class EnhancedLLMInterface(LLMInterface):
    ...
    def _check_ollama_setup(self) -> bool:
        ...
        try:
            ...
            if version_result.returncode == 0:
                ...
                raise RuntimeError(f"Failed to get Ollama version: {version_result.stderr}")
            ...
            if show_result.returncode != 0:
                ...
                if pull_result.returncode != 0:
                    raise RuntimeError(f"Failed to pull model '{model_name}': {pull_result.stderr}")
            ...
            raise RuntimeError("Ollama not found. Please install it using 'brew install ollama'")
    ...
    async def acall_llm(self, prompt: str, system_prompt: Optional[str] = None, **kwargs) -> Tuple[str, Dict[str, int]]:
    ...
    def call_llm(self, prompt: str, system_prompt: Optional[str] = None, **kwargs) -> Tuple[str, Dict[str, int]]:
        ...
        for attempt in range(self.system_config.max_retries):
            try:
                ...
                if process.returncode != 0:
                    stderr_msg = process.stderr.decode(errors='replace') if process.stderr else "Unknown error"
                ...
                self.logger.error(f"Attempt {attempt + 1} failed: Timeout after {self.model_config.timeout}s for prompt: {prompt[:100]}...")
                if attempt == self.system_config.max_retries - 1:
                    raise RuntimeError(f"LLM call failed after {self.system_config.max_retries} attempts due to timeout.")
