...
class QualityControl:
    ...
    async def run_quality_checks(self, state: BookState) -> BookState:
        ...
        try:
            ...
            # Step 2: Proofreading
            state = await self.proofreader.execute(state)
            ...
            if not self._meets_quality_standards(quality_scores):
                state.metadata['improvement_plan'] = await self._generate_improvement_plan(state)
    ...
    async def _perform_technical_review(self, state: BookState) -> None:
        ...
        for chapter_title, content in state.chapter_map.items():
            ...
            try:
                review_result, _ = await self.llm.call_llm(prompt)
    ...
    async def _generate_improvement_plan(self, state: BookState) -> Dict[str, Any]:
        ...
        low_quality_areas = [
            metric for metric, score in state.metadata['quality_scores'].items()
        ...
        for area in low_quality_areas:
            ...
            try:
                response, _ = await self.llm.call_llm(prompt)
                improvement_plan['specific_actions'].append({
                    'area': area,
                    'recommendations': response
                })
            except Exception as e:
                self.logger.warning(f"Failed to generate improvement plan for {area}: {e}")
    ...
    def _summarize_readability(self, state: BookState) -> Dict[str, Any]:
        ...
        return {
            'average_scores': {
                metric: sum(ch[metric] for ch in readability.values()) / len(readability)
                for metric in next(iter(readability.values())).keys()
            },
            'problematic_chapters': [
                chapter for chapter, scores in readability.items()
