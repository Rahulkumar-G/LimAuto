...
class ContentValidator:
    ...
    async def _check_readability(self, state: BookState) -> Dict[str, Any]:
        ...
        for chapter, content in state.chapter_map.items():
            ...
            if not (target['min'] <= score <= target['max']):
                issues.append(f"Chapter '{chapter}' readability ({score:.1f}) outside target range")
        ...
        return {
            ...
            'score': avg_score / 100,
            'issues': issues,
            'recommendations': self._get_readability_recommendations(issues, state.target_audience)
        }
    ...
    def _get_readability_recommendations(self, issues: List[str], target_audience: str) -> List[str]:
